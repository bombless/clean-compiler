...
checkInstanceType :: !Index !GlobalIndex !ClassIdent !ClassArgs !BITVECT
	...
	-> (!InstanceType,!Specials,!u:{# CheckedTypeDef},!v:{# ClassDef},!u:{# DclModule},!*TypeHeaps,!*CheckState)
checkInstanceType mod_index ins_class_index ins_class_ident class_args class_fun_dep_vars it=:{it_types,it_context} specials type_defs class_defs modules heaps cs
	# ots = { ots_type_defs = type_defs, ots_modules = modules }
	  ...
	  (it_types, undefined_contexts_vars, (ots, oti=:{oti_all_vars = it_vars, oti_all_attrs = it_attr_vars}, cs))
		= check_instance_type class_fun_dep_vars mod_index it_types (ots, oti, cs)
	  (heaps, cs) = IF_ALLOW_NON_LINEAR_INSTANCES
						(oti.oti_heaps, cs)
						(check_linearity_of_type_vars it_vars class_args it_types oti.oti_heaps cs)
...
checkMemberType mod_index st type_defs class_defs modules heaps cs
	...
	where
		add_attrs_to_class_vars [TV type_var=:{tv_info_ptr}:tv_type_vars] th_vars
			...
			= (AClassArg {atv_attribute = attr, atv_variable = type_var} atype_class_vars,th_vars)
		add_attrs_to_class_vars [CV type_var=:{tv_info_ptr} :@: pattern_args:tv_type_vars] th_vars
			# (TVI_AttrAndRefCount attr _, th_vars) = readPtr tv_info_ptr th_vars
			# (atype_class_vars,th_vars) = add_attrs_to_class_vars tv_type_vars th_vars
			// to do: attributes in pattern_arg_vars ?
			# pattern_arg_vars = [{atv_attribute=at_attribute,atv_variable=ptv} \\ {at_attribute,at_type=TV ptv}<-pattern_args]
			= (AClassArgPattern {atv_attribute = attr, atv_variable = type_var} pattern_arg_vars atype_class_vars,th_vars)
		add_attrs_to_class_vars [] th_vars
			= (ANoClassArgs,th_vars)
...
checkSuperClasses class_args class_contexts mod_index type_defs class_defs modules heaps=:{th_vars} cs=:{cs_symbol_table,cs_error}
	# (class_args, symbol_ptrs, cs_symbol_table, th_vars, cs_error)
	  ...
	  (class_contexts, fun_dep_determined_vars, type_defs, class_defs, modules, type_heaps, cs)
				= checkTypeContexts class_contexts [] [] mod_index class_defs ots oti cs
	  cs = check_class_variables fun_dep_determined_vars cs
...
checkInstanceTypeContexts tcs instance_type_vars instance_type undefined_contexts_vars mod_index class_defs ots oti cs
	| cs.cs_x.x_allow_undecidable_instances
		= checkUndecidableInstanceTypeContexts tcs instance_type_vars instance_type undefined_contexts_vars mod_index class_defs ots oti cs
	# (instance_type_ref_counts,instance_type_size,oti,cs)
	  ...
	  cs & cs_symbol_table=cs_symbol_table
	  fun_dep_determined_vars = if (new_fun_dep_determined_vars=:[]) fun_dep_determined_vars (fun_dep_determined_vars++reverse new_fun_dep_determined_vars)
	  (free_vars,cs) = collect_free_contexts_vars_error undefined_contexts_vars oti.oti_all_vars cs
	  cs = add_errors_for_type_variables_only_in_constraints_and_dependent_part fun_dep_determined_vars cs
...
checkUndecidableInstanceTypeContexts :: ![TypeContext] ![TypeVar] ![Type] ![[TypeVar]] !Index !v:{#ClassDef} !u:OpenTypeSymbols !*OpenTypeInfo !*CheckState
	-> (![TypeContext], ![TypeVar], !u:{#CheckedTypeDef}, !v:{#ClassDef}, u:{#DclModule}, !*TypeHeaps, !*CheckState)
checkUndecidableInstanceTypeContexts tcs instance_type_vars instance_type undefined_contexts_vars mod_index class_defs ots oti cs
	# (oti,cs) = zero_ref_counts_of_instance_type tcs instance_type_vars oti cs
	  (tcs, fun_dep_determined_vars, undefined_contexts_vars, fun_contexts_vars, (class_defs, {ots_modules,ots_type_defs}, oti, cs))
		= check_type_contexts tcs [] undefined_contexts_vars [] instance_type_vars mod_index (class_defs, ots, oti, cs)
	  (new_fun_dep_determined_vars,cs_symbol_table) = iterate_define_fun_contexts_dep_vars fun_contexts_vars [] cs.cs_symbol_table
	  cs & cs_symbol_table=cs_symbol_table
	  fun_dep_determined_vars = if (new_fun_dep_determined_vars=:[]) fun_dep_determined_vars (fun_dep_determined_vars++reverse new_fun_dep_determined_vars)
	  (free_vars,cs) = collect_free_contexts_vars_error undefined_contexts_vars oti.oti_all_vars cs
	...
	= (tcs, fun_dep_determined_vars ++ free_vars, ots_type_defs, class_defs, ots_modules, oti.oti_heaps, cs)
where
	check_type_contexts [tc:tcs] fun_dep_determined_vars undefined_contexts_vars fun_contexts_vars instance_type_vars mod_index (class_defs, ots, oti, cs)
		# (tc,free_vars,free_dep_vars,fun_dep_determined_vars,(class_defs, ots, oti, cs))
			= check_type_context mod_index tc fun_dep_determined_vars (class_defs, ots, oti, cs)
		  (undefined_contexts_vars,fun_contexts_vars)
			= update_undefined_and_fun_contexts_vars free_vars free_dep_vars undefined_contexts_vars fun_contexts_vars
		  (oti,cs) = zero_ref_counts instance_type_vars oti cs
		  (tcs,fun_dep_determined_vars,undefined_contexts_vars,fun_contexts_vars,class_defs_ots_oti_cs)
			= check_type_contexts tcs fun_dep_determined_vars undefined_contexts_vars fun_contexts_vars instance_type_vars mod_index (class_defs, ots, oti, cs)
		= ([tc:tcs],fun_dep_determined_vars,undefined_contexts_vars,fun_contexts_vars,class_defs_ots_oti_cs)
	check_type_contexts [] fun_dep_determined_vars undefined_contexts_vars fun_contexts_vars instance_type_vars mod_index class_defs_ots_oti_cs
		= ([],fun_dep_determined_vars,undefined_contexts_vars,fun_contexts_vars,class_defs_ots_oti_cs)

	zero_ref_counts_of_instance_type [] instance_type_vars oti cs
		= (oti,cs)
	zero_ref_counts_of_instance_type _ instance_type_vars oti cs
		= zero_ref_counts instance_type_vars oti cs
	...
	zero_ref_counts [{tv_ident={id_info}}:free_vars] oti=:{oti_heaps} cs=:{cs_symbol_table}
		# (entry=:{ste_kind},cs_symbol_table) = readPtr id_info cs_symbol_table
		  ...
		  th_vars = writePtr tv_info_ptr (TVI_AttrAndRefCount attr 0) th_vars
		  oti & oti_heaps = {oti_heaps & th_vars = th_vars}
		= zero_ref_counts free_vars oti cs
	zero_ref_counts [] oti cs
		= (oti,cs)
