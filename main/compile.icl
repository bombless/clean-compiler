		...
		= case optionalSyntaxTree of
			Yes syntaxTree
				# functions_and_macros = syntaxTree.fe_icl.icl_functions
				  (Yes path) = mbModPath
				  (csf_dir_exists,abc_file_path,error,files)
					= if (size options.outputPathName==0)
						(abc_file_name_in_clean_system_files_folder path options.moduleName error files)
						(True,options.outputPathName,error,files)
